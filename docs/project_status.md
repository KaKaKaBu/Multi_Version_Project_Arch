...
| 文档 | **高（~90%）** | 架构/维护/完成度已同步，并覆盖 Flutter 主上位机、可选 uni-app 小程序栈、`version_features.json`、导出目录结构 |
...
| 能力 | 状态 |
| --- | --- |
| `APP_VERSION` 1–10 条件编译 + `VERSION_FEATURE_*` + `driver_catalog` 选驱动 | ✅ |
...
| 工具 | 路径 | 状态 |
| --- | --- | --- |
| `gen_project.py` | `tools/` | ✅ 生成应用骨架 + 默认 Flutter 上位机 scaffold；传入 `--with-mini-program` 时额外生成 uni-app/Capacitor 小程序 scaffold（含 `src/features/`、`version_features.json`） |
...
| 低 | 历史 uni-app-only 项目 | DCLD_001 / ZNCZ_001 等历史上位机可按交付需要单独迁移到 Flutter |
