...
hal_status_t adc_hal_init(const adc_hal_config_t *cfg)
{
    ...
    adc_hal_runtime.instance = cfg->instance;
    ...
    ADC_Init(cfg->instance, &adc);
    ADC_Cmd(cfg->instance, ENABLE);

    if (adc_hal_calibrate(cfg->instance, adc_hal_runtime.timeout_us) != HAL_OK) {
