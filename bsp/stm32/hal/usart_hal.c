...
hal_status_t usart_hal_init(const usart_hal_config_t *cfg)
{
    ...
    USARTx = stm32f1_usart(cfg->instance);
    if (USARTx == 0) {
        return HAL_ERR_PARAM;
    }

    runtime = usart_hal_runtime_get(USARTx);
    ...
    if ((cfg->tx_mode == USART_HAL_TX_MODE_DMA) && (cfg->tx_dma_channel != 0)) {
        hal_status_t status = usart_hal_dma_setup(USARTx, stm32f1_dma_channel(cfg->tx_dma_channel));
        if (status != HAL_OK) {
            return status;
        }
    }
...
{
    USART_TypeDef *USARTx = stm32f1_usart(instance);
 ...
 */
static hal_status_t usart_hal_send_byte_native(USART_TypeDef *USARTx, uint8_t data)
{
    usart_hal_runtime_t *runtime = usart_hal_runtime_get(USARTx);
    usart_hal_flag_ctx_t ctx;
    hal_status_t status;
...
static hal_status_t usart_hal_send_buffer_irq(USART_TypeDef *USARTx, const uint8_t *data, uint16_t len)
{
    ...
    for (i = 0; i < len; ++i) {
        status = usart_hal_send_byte_native(USARTx, data[i]);
        if (status != HAL_OK) {
            return status;
        }
    }
...
hal_status_t usart_hal_read(hal_usart_id_t instance, uint8_t *buf, uint16_t len)
{
    ...
    for (i = 0; i < len; ++i) {
        received = usart_hal_recv_byte(instance, &buf[i]);
        if (received <= 0) {
