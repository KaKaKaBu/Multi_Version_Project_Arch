...
#define APP_MODE_TIMER 1U

#define APP_MANUAL_VIEW_MAIN 0U
#define APP_MANUAL_VIEW_WIFI 1U
...
typedef struct app_context {
    ...
    unsigned char mode;
    unsigned char manual_view;
    unsigned char wifi_page;
...
{
    if (app_ctx.mode != APP_MODE_TIMER) {
        return;
    }
...
static void app_display_manual_wifi(void)
{
    ...
    if (app_ctx.wifi_page == APP_WIFI_PAGE_PASS) {
        title = "WiFi PASS";
        value = BOARD_ESP8266_WIFI_PASS;
    } else if (app_ctx.wifi_page == APP_WIFI_PAGE_STATUS) {
        title = "WiFi STAT";
...
static void app_display_refresh(void)
{
    ...
    if (app_ctx.manual_view == APP_MANUAL_VIEW_WIFI) {
...
static void app_handle_key_manual(unsigned char key_id)
{
    if (app_ctx.manual_view == APP_MANUAL_VIEW_WIFI) {
        if (key_id == 2U) {
            app_ctx.manual_view = APP_MANUAL_VIEW_MAIN;
            app_ctx.display_dirty = 1U;
            return;
        }
        if (key_id == 3U) {
            app_ctx.wifi_page = APP_WIFI_PAGE_SSID;
            app_ctx.display_dirty = 1U;
            return;
        }
        if (key_id == 4U) {
            app_ctx.wifi_page = APP_WIFI_PAGE_PASS;
            app_ctx.display_dirty = 1U;
            return;
        }
        if (key_id == 5U) {
            app_ctx.wifi_page = APP_WIFI_PAGE_STATUS;
            app_ctx.display_dirty = 1U;
        }
        return;
    }
    ...
    if (key_id == 2U) {
        app_relay_apply(app_ctx.relay_state == 0U ? 1U : 0U);
        return;
    }
    if (key_id == 3U) {
        app_ctx.manual_view = APP_MANUAL_VIEW_WIFI;
        app_ctx.wifi_page = APP_WIFI_PAGE_SSID;
        app_ctx.display_dirty = 1U;
        return;
    }
...
static void app_handle_key_timer(unsigned char key_id)
{
    app_hms_t *field_time;

    if (key_id == 2U) {
        app_ctx.timer_field = (unsigned char)((app_ctx.timer_field + 1U) % 3U);
        app_ctx.display_dirty = 1U;
        return;
    }
    if (key_id == 3U) {
        app_ctx.timer_digit = (unsigned char)((app_ctx.timer_digit + 1U) % 3U);
        app_ctx.display_dirty = 1U;
        return;
    }

    if (app_ctx.timer_field == APP_TIMER_FIELD_CURRENT) {
        if (key_id == 4U) {
            app_adjust_current_digit(app_ctx.timer_digit, 1);
            app_rtc_write_now();
    ...
    field_time = app_timer_field_hms(app_ctx.timer_field);

    if (key_id == 4U) {
        app_adjust_digit(field_time, app_ctx.timer_digit, 1);
        app_timer_apply_relay();
        app_ctx.display_dirty = 1U;
        return;
    }
...
void app_logic_on_key(unsigned char key_id)
{
    ...
    if (app_ctx.mode == APP_MODE_TIMER) {
        app_handle_key_timer(key_id);
        return;
    }

    app_handle_key_manual(key_id);
}
...
static void app_publish_status(void)
{
    ...
    mode_text = (app_ctx.mode == APP_MODE_TIMER) ? "timer" : "manual";
...
{
    app_rtc_read_now();
    DEBUG_LOG_RTC("[RTC] app now=%04u-%02u-%02u %02u:%02u:%02u mode=%u dirty=1\r\n",
