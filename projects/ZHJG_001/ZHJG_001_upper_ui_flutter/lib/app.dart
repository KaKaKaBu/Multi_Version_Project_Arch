...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    super.initState();
    service = ZhjgService(transport: transport, capabilities: capabilities);
    _connSub = transport.onConnectionChanged.listen((connected) {
  ...
  Widget build(BuildContext context) {
    final remoteEnabled =
        capabilities.has('wifi') || capabilities.has('app') || capabilities.has('ble');
    return Scaffold(
      appBar: AppBar(
        title: const Text('ZHJG_001 智能井盖'),
        actions: [
          IconButton(
        ...
        children: [
          ...
          Expanded(
            child: IndexedStack(
              ...
              children: [
                DashboardPage(service: service, capabilities: capabilities),
      ...
      bottomNavigationBar: NavigationBar(
        ...
        destinations: const [
          ...
          NavigationDestination(icon: Icon(Icons.tune), label: '阈值'),
        ],
      ),
    );
  }
