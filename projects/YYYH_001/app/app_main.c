...
static sched_loop_t sensor_loop = SCHED_LOOP_DEF(
    "sensor", sensor_loop_run, 3U, 800U, APP_EVENT_RTC, APP_EVENT_SENSOR);

static sched_loop_t alarm_loop = SCHED_LOOP_DEF(
    "alarm", alarm_loop_run, 2U, 120U, APP_EVENT_RTC | APP_EVENT_SENSOR | APP_EVENT_KEY, 0U);

static sched_loop_t display_loop = SCHED_LOOP_DEF(
    "display", display_loop_run, 5U, 300U,
    APP_EVENT_RTC | APP_EVENT_SENSOR | APP_EVENT_KEY | APP_EVENT_ALARM | APP_EVENT_COMM_RX,
    0U);
...
void app_comm_poll(sched_event_t events)
{
    ...
    if (esp8266_mqtt_is_ready() != 0) {
        if ((events & (APP_EVENT_COMM_RX | APP_EVENT_TICK | APP_EVENT_RTC)) != 0U) {
            esp8266_mqtt_poll();
        }
        return;
    }

    if ((events & (APP_EVENT_TICK | APP_EVENT_RTC)) == 0U) {
        return;
    }
