...
export const versionFeatureMatrix = {
  ...
  8: ['common', 'dht11', 'mq2', 'remote', 'wifi', 'cloud', 'app'],
  ...
  14: ['common', 'dht11', 'mq2', 'mq7', 'remote', 'wifi', 'cloud', 'app'],
}
...
export function getSupportedTransports(features = resolveUpperFeatures().flags) {
  const transports = []
  if (features.wifi) transports.push({ key: 'mqtt', label: features.cloud ? '华为云 MQTT' : 'WiFi MQTT' })
...
export function describeVersion(version = resolveUpperVersion(), features = resolveUpperFeatures(version).flags) {
  ...
  if (features.wifi) parts.push(features.cloud ? '华为云' : features.mpWeixin ? '微信小程序' : features.web ? '网页' : 'WiFi App')
  if (features.ble) parts.push('蓝牙 App')
  return `KQZL2-${String(version).padStart(3, '0')} · ${parts.join(' + ')}`
}
