...
class MqttTransport extends TransportService {
  ...
  Future<bool> connect(Map<String, String> _) async {
    ...
    final cameraTopic = config.cameraTopic;
    if (cameraTopic != null &&
        cameraTopic.isNotEmpty &&
        cameraTopic != config.telemetryTopic) {
      client.subscribe(cameraTopic, MqttQos.atMostOnce);
    }
  ...
  void _handleMessages(List<MqttReceivedMessage<MqttMessage>> messages) {
    for (final message in messages) {
      if (!_isSubscribedDataTopic(message.topic) ||
  ...
  bool _isSubscribedDataTopic(String topic) {
    return topic == config.telemetryTopic || topic == config.cameraTopic;
  }
